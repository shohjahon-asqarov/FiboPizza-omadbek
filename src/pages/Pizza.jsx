...
const Pizza = ({cart, setCart, addCart}) => {
  ...
  return (
    <section className='py-10'>
      <div className="w-full max-w-[1240px] px-5 mx-auto">
        <h2 className='font-bold py-10 text-[32px] text-yellow leading-10'>Pizza</h2>
            ...
            pitsa.map((piza) => {
              return (
                <li data-aos="fade-up" className='p-5 border border-border rounded-lg hover:shadow-shadow flex flex-col items-center justify-center'>
                  <button className='disabled sm:cursor-pointer' onClick={() => openModal(piza) }>
                  ...
                  <div className="w-full flex items-center justify-between">
                    ...
                    <button onClick={() => addCart(piza)} className='w-32 py-1 right-full bg-yellow text-center rounded-lg font-bold text-sm leading-7 text-white'>{piza.btn}</button>
                  </div>
                </li>
              )
            })
